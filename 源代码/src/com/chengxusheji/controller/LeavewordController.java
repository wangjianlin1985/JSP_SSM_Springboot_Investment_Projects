...
public class LeavewordController extends BaseController {
	...
	public void companyAdd(Leaveword leaveword, BindingResult br,
		...
		leaveword.setCompanyObj(company);
	...
	public void list(String leaveTitle,@ModelAttribute("companyObj") Company companyObj,String leaveTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
		if(rows != 0)leavewordService.setRows(rows);
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, companyObj, leaveTime, page);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, companyObj, leaveTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String leaveTitle,@ModelAttribute("companyObj") Company companyObj,String leaveTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, companyObj, leaveTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, companyObj, leaveTime);
	    /*获取到总的页码数目*/
	    int totalPage = leavewordService.getTotalPage();
	...
	public String companyFrontlist(String leaveTitle,@ModelAttribute("companyObj") Company companyObj,String leaveTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
	    ...
	    List<Company> companyList = companyService.queryAllCompany();
	    request.setAttribute("companyList", companyList);
		return "Leaveword/leaveword_companyFrontquery_result"; 
	}
	...
	public void OutToExcel(String leaveTitle,@ModelAttribute("companyObj") Company companyObj,String leaveTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(leaveTitle == null) leaveTitle = "";
        if(leaveTime == null) leaveTime = "";
        List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle,companyObj,leaveTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Leaveword信息记录"; 
        String[] headers = { "留言id","留言标题","留言单位","留言时间","管理回复","回复时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<leavewordList.size();i++) {
        	Leaveword leaveword = leavewordList.get(i); 
        	dataset.add(new String[]{leaveword.getLeaveWordId() + "",leaveword.getLeaveTitle(),leaveword.getCompanyObj().getCompanyName(),leaveword.getLeaveTime(),leaveword.getReplyContent(),leaveword.getReplyTime()});
        }
        /*
