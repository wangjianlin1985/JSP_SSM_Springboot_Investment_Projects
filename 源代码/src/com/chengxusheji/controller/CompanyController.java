...
public class CompanyController extends BaseController {
	...
	public void add(Company company, BindingResult br,
		...
		company.setRegTime(sdf.format(new java.util.Date()));
	...
	public void list(String companyUserName,String companyName,String gszch,String gsxz,@ModelAttribute("hangyeObj") Hangye hangyeObj,String lxr,String lxdh,String shzt,String regTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (companyUserName == null) companyUserName = "";
		...
		List<Company> companyList = companyService.queryCompany(companyUserName, companyName, gszch, gsxz, hangyeObj, lxr, lxdh, shzt, regTime, page);
	    /*计算总的页数和总的记录数*/
	    companyService.queryTotalPageAndRecordNumber(companyUserName, companyName, gszch, gsxz, hangyeObj, lxr, lxdh, shzt, regTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String companyUserName,String companyName,String gszch,String gsxz,@ModelAttribute("hangyeObj") Hangye hangyeObj,String lxr,String lxdh,String shzt,String regTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		List<Company> companyList = companyService.queryCompany(companyUserName, companyName, gszch, gsxz, hangyeObj, lxr, lxdh, shzt, regTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    companyService.queryTotalPageAndRecordNumber(companyUserName, companyName, gszch, gsxz, hangyeObj, lxr, lxdh, shzt, regTime);
	    /*获取到总的页码数目*/
	    int totalPage = companyService.getTotalPage();
	    ...
	    request.setAttribute("companyUserName", companyUserName);
	...
	public void OutToExcel(String companyUserName,String companyName,String gszch,String gsxz,@ModelAttribute("hangyeObj") Hangye hangyeObj,String lxr,String lxdh,String shzt,String regTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(companyUserName == null) companyUserName = "";
        if(companyName == null) companyName = "";
        if(gszch == null) gszch = "";
        if(gsxz == null) gsxz = "";
        ...
        if(shzt == null) shzt = "";
        if(regTime == null) regTime = "";
        List<Company> companyList = companyService.queryCompany(companyUserName,companyName,gszch,gsxz,hangyeObj,lxr,lxdh,shzt,regTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Company信息记录"; 
        String[] headers = { "企业用户名","企业名称","工商注册号","营业执照","公司性质","公司规模","公司行业","联系人","联系电话","审核状态","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<companyList.size();i++) {
        	Company company = companyList.get(i); 
        	dataset.add(new String[]{company.getCompanyUserName(),company.getCompanyName(),company.getGszch(),company.getYyzz(),company.getGsxz(),company.getGsgm(),company.getHangyeObj().getHangyeName(),company.getLxr(),company.getLxdh(),company.getShzt(),company.getRegTime()});
        }
        /*
