package com.chengxusheji.service;

import java.util.ArrayList;
import javax.annotation.Resource; 
import org.springframework.stereotype.Service;

import com.chengxusheji.po.Admin;
import com.chengxusheji.po.Hangye;
import com.chengxusheji.po.Company;

import com.chengxusheji.mapper.CompanyMapper;
@Service
public class CompanyService {

	@Resource CompanyMapper companyMapper;
    /*每页显示记录数目*/
    private int rows = 10;;
    public int getRows() {
		return rows;
	}
	public void setRows(int rows) {
		this.rows = rows;
	}

    /*保存查询后总的页数*/
    private int totalPage;
    public void setTotalPage(int totalPage) {
        this.totalPage = totalPage;
    }
    public int getTotalPage() {
        return totalPage;
    }

    /*保存查询到的总记录数*/
    private int recordNumber;
    public void setRecordNumber(int recordNumber) {
        this.recordNumber = recordNumber;
    }
    public int getRecordNumber() {
        return recordNumber;
    }

    /*添加企业记录*/
    public void addCompany(Company company) throws Exception {
    	companyMapper.addCompany(company);
    }

    /*按照查询条件分页查询企业记录*/
    public ArrayList<Company> queryCompany(String companyUserName,String companyName,String gszch,String gsxz,Hangye hangyeObj,String lxr,String lxdh,String shzt,String regTime,int currentPage) throws Exception { 
     	String where = "where 1=1";
    	if(!companyUserName.equals("")) where = where + " and t_company.companyUserName like '%" + companyUserName + "%'";
    	if(!companyName.equals("")) where = where + " and t_company.companyName like '%" + companyName + "%'";
    	if(!gszch.equals("")) where = where + " and t_company.gszch like '%" + gszch + "%'";
    	if(!gsxz.equals("")) where = where + " and t_company.gsxz like '%" + gsxz + "%'";
    	if(null != hangyeObj && hangyeObj.getHangyeId()!= null && hangyeObj.getHangyeId()!= 0)  where += " and t_company.hangyeObj=" + hangyeObj.getHangyeId();
    	if(!lxr.equals("")) where = where + " and t_company.lxr like '%" + lxr + "%'";
    	if(!lxdh.equals("")) where = where + " and t_company.lxdh like '%" + lxdh + "%'";
    	if(!shzt.equals("")) where = where + " and t_company.shzt like '%" + shzt + "%'";
    	if(!regTime.equals("")) where = where + " and t_company.regTime like '%" + regTime + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return companyMapper.queryCompany(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<Company> queryCompany(String companyUserName,String companyName,String gszch,String gsxz,Hangye hangyeObj,String lxr,String lxdh,String shzt,String regTime) throws Exception  { 
     	String where = "where 1=1";
    	if(!companyUserName.equals("")) where = where + " and t_company.companyUserName like '%" + companyUserName + "%'";
    	if(!companyName.equals("")) where = where + " and t_company.companyName like '%" + companyName + "%'";
    	if(!gszch.equals("")) where = where + " and t_company.gszch like '%" + gszch + "%'";
    	if(!gsxz.equals("")) where = where + " and t_company.gsxz like '%" + gsxz + "%'";
    	if(null != hangyeObj && hangyeObj.getHangyeId()!= null && hangyeObj.getHangyeId()!= 0)  where += " and t_company.hangyeObj=" + hangyeObj.getHangyeId();
    	if(!lxr.equals("")) where = where + " and t_company.lxr like '%" + lxr + "%'";
    	if(!lxdh.equals("")) where = where + " and t_company.lxdh like '%" + lxdh + "%'";
    	if(!shzt.equals("")) where = where + " and t_company.shzt like '%" + shzt + "%'";
    	if(!regTime.equals("")) where = where + " and t_company.regTime like '%" + regTime + "%'";
    	return companyMapper.queryCompanyList(where);
    }

    /*查询所有企业记录*/
    public ArrayList<Company> queryAllCompany()  throws Exception {
        return companyMapper.queryCompanyList("where 1=1");
    }

    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String companyUserName,String companyName,String gszch,String gsxz,Hangye hangyeObj,String lxr,String lxdh,String shzt,String regTime) throws Exception {
     	String where = "where 1=1";
    	if(!companyUserName.equals("")) where = where + " and t_company.companyUserName like '%" + companyUserName + "%'";
    	if(!companyName.equals("")) where = where + " and t_company.companyName like '%" + companyName + "%'";
    	if(!gszch.equals("")) where = where + " and t_company.gszch like '%" + gszch + "%'";
    	if(!gsxz.equals("")) where = where + " and t_company.gsxz like '%" + gsxz + "%'";
    	if(null != hangyeObj && hangyeObj.getHangyeId()!= null && hangyeObj.getHangyeId()!= 0)  where += " and t_company.hangyeObj=" + hangyeObj.getHangyeId();
    	if(!lxr.equals("")) where = where + " and t_company.lxr like '%" + lxr + "%'";
    	if(!lxdh.equals("")) where = where + " and t_company.lxdh like '%" + lxdh + "%'";
    	if(!shzt.equals("")) where = where + " and t_company.shzt like '%" + shzt + "%'";
    	if(!regTime.equals("")) where = where + " and t_company.regTime like '%" + regTime + "%'";
        recordNumber = companyMapper.queryCompanyCount(where);
        int mod = recordNumber % this.rows;
        totalPage = recordNumber / this.rows;
        if(mod != 0) totalPage++;
    }

    /*根据主键获取企业记录*/
    public Company getCompany(String companyUserName) throws Exception  {
        Company company = companyMapper.getCompany(companyUserName);
        return company;
    }

    /*更新企业记录*/
    public void updateCompany(Company company) throws Exception {
        companyMapper.updateCompany(company);
    }

    /*删除一条企业记录*/
    public void deleteCompany (String companyUserName) throws Exception {
        companyMapper.deleteCompany(companyUserName);
    }

    /*删除多条企业信息*/
    public int deleteCompanys (String companyUserNames) throws Exception {
    	String _companyUserNames[] = companyUserNames.split(",");
    	for(String _companyUserName: _companyUserNames) {
    		companyMapper.deleteCompany(_companyUserName);
    	}
    	return _companyUserNames.length;
    }
	
	
	/*保存业务逻辑错误信息字段*/
	private String errMessage;
	public String getErrMessage() { return this.errMessage; }
	
	/*验证用户登录*/
	public boolean checkLogin(String userName, String password) throws Exception { 
		Company db_company = (Company) companyMapper.getCompany(userName);
		if(db_company == null) { 
			this.errMessage = " 账号不存在 ";
			System.out.print(this.errMessage);
			return false;
		} else if( !db_company.getPassword().equals(password)) {
			this.errMessage = " 密码不正确! ";
			System.out.print(this.errMessage);
			return false;
		} else if( db_company.getShzt().equals("待审核")) {
			this.errMessage = "你还没通过管理审核! ";
			System.out.print(this.errMessage);
			return false;
		}
		
		return true;
	}
	
	
}
